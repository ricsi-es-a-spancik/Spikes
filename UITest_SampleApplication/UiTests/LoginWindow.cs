namespace UiTests
{
    ...
    public class LoginWindow
    {
        ...
        public LoginWindow(Func<Window> getWindow)
        {
        ...
        private TextBox LoginNameTextBox => _getWindow().Get<TextBox>(SearchCriteria.Indexed(0));

        private TextBox PasswordTextBox => _getWindow().Get<TextBox>(SearchCriteria.Indexed(1));

        private Button LoginButton => _getWindow().Get<Button>(SearchCriteria.ByText("Login"));

        private Button CancelButton => _getWindow().Get<Button>(SearchCriteria.ByText("Cancel"));

        private Label CredentialsErrorMessageLabel => _getWindow().Get<Label>(SearchCriteria.ByText(CREDENTIALS_ERROR_MESSAGE));
